---
description: Investiga (bug de campo, feature nueva, incógnita técnica) y produce un expediente + issues enjambre-listos que el fleet pueda consumir sin diseñar en abierto.
---

# /kelpie-research — investigación → issues enjambre-listos

`$ARGUMENTS`: la pregunta, el bug o la feature.

Los 53 issues del roadmap ya vienen enriquecidos. Este comando es para **todo lo que nazca fuera de
...
2. **Issues propuestos** con el formato de `/kelpie-issue`, cada criterio binario y cada referencia
   citada, más el bloque que el fleet necesita: **dominio** (labels `area:*`) y **depende de #N**.
